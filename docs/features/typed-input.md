...
> **Status (2026-05-30):** **Shipped — 33A (per-property `default:`, literal + shell), 33B (type-aware `{{slot}}` rendering), 33E (`variables:` folded into the schema slot space with a one-time warning).** This means schema-declared defaults now fill `{{name}}` slots, arrays/objects render as compact JSON (pretty opt-in), and `-v` overrides a schema default. **Not yet shipped:** **33C** — `-v` type *coercion* against the declared type (today `-v` values are still strings until coerced) and stdin routing via `x-aichat: { source: stdin }`; and **33D** — strict adjacent-stage pipeline shape-checking. Sections below that describe 33C (CLI/stdin coercion, the `@file.json` form, the `body` stdin slot) and 33D (pipeline shape-check) document the target shape; they land in the next two commits. The reserved namespaces (`{{__sys__}}`, `{{$AICHAT_*}}`, `{{.field}}`) already work identically everywhere.
