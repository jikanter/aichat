...
> **Status:** This doc describes the target shape introduced by [Phase 33](../roadmap/phase-33-overview.md). Until Phase 33 ships, the only fully-supported authoring path is `variables:` (string-only) plus `input_schema:` (validation only) as two parallel declarations. The reserved namespaces (`{{__sys__}}`, `{{$AICHAT_*}}`, `{{.field}}`) work identically on both paths.
